...
class Main {
	public static void main(String[] args) {
		...
		while(functions.size() != 0) {
			...
			for (int i = 0; i < kValues.size(); i++) {
				optimizeFunction(functions.get(0), new Integer(args[0]), kValues.get(i), new Boolean(args[1]));
			}
	...
	public static void optimizeFunction(OptimizationFunction function, int numDimensions, double kValue, boolean showProgress) {
		...
		for (int i = 0; i < numRunsPerFunction; i++) { // execute function 30 times.
			...
			CooperativeDE coopDE = new CooperativeDE(crossover, mutation, function, numInstances, numDimensions);
