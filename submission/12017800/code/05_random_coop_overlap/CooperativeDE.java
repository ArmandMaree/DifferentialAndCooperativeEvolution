...
public class CooperativeDE {
	...
	public CooperativeDE(Crossover crossover, Mutation mutation, OptimizationFunction optimizationFunction, int numInstances, int numDimensions, double overlapProbability) {
		...
		while (currentIteration < maxIteration && !Double.isInfinite(bestFitness)) { //  && noImprovementIteration < maxNoImprovementIteration
			...
			for (int i = 0; i < numInstances; i++) {
				...
				for (int j = 0; j < numDimensions; j++) {
					if (randomGenerator.nextDouble() < overlapProbability) {
						indicesForDe.add(j);
					}
				}
