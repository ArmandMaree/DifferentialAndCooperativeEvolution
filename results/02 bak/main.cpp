...
int optimizeFunction(OptimizationFunction* function, int numDimensions, float kValue);
...
int main(int argc, char const *argv[]) {
	...
	kValues.push_back(0.1);
	kValues.push_back(0.3);
	kValues.push_back(0.5);
	...
	while(functions.size() != 0) {
		...
		for (int i = 0; i < kValues.size(); i++) {
			if (optimizeFunction(functions[0], atoi(argv[1]), kValues[i]) == 1) {
				return 1;
			}
		}
...
int optimizeFunction(OptimizationFunction* function, int numDimensions, float kValue) {
	...
	int numDimensionsPerInstance = numDimensions * kValue;
	int numInstances = numDimensions / numDimensionsPerInstance;
	cout << "numDimensions: " << numDimensions << endl;
	cout << "numDimensionsPerInstance: " << numDimensionsPerInstance << endl;
